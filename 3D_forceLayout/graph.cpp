// use 0.0002 for stable equalibrium
// use 0.0005 for spiral 
#include <stdio.h>
#include <iostream>
...
void preprocessEvents(){
	...
	if (KEYS['e']){
   	...
   	if (KEYS['n']){
		graphTime += 0.001; 
		graph.stepGraphCoordinates(STEPPING_RATE); 
	}	
   	if (KEYS['b']){
		graphTime += 0.001; 
		graph.stepGraphCoordinates(STEPPING_RATE); 
...
void loadGraphFromDatafile(Graph &graph, string datafilename){
	...
	if (inputfile){
		...
		while(getline(inputfile,line)){
			...
			float zCoordinate = stof(thirdNumber); 	
			// create a new node with specific coordinate
			Node newNode(xCoordinate, yCoordinate, zCoordinate); 
